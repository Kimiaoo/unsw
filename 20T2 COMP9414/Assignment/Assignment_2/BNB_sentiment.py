# @file: BNB_sentiment.py
# @author: Hongxiao Jin
# @creat_time: 2020/7/23 17:56
...
count = CountVectorizer(token_pattern=r'[#@_$%\b\w\w+\b]{2,}', lowercase=False)
...
# ------------------------------------- Bernoulli Naive Bayes set model ------------------------------------
clf = BernoulliNB()
model = clf.fit(tweet_train_bag_of_words, train_sentiment)

# ------------------------------------- print results -------------------------------------
BNB_predicted_sentiment = model.predict(tweet_test_bag_of_words)
for i in range(0, len(BNB_predicted_sentiment)):
    print(test_instance_number[i], BNB_predicted_sentiment[i])

# print(classification_report(test_sentiment, BNB_predicted_sentiment, zero_division=0))
