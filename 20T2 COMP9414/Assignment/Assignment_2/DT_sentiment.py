...
count = CountVectorizer(token_pattern=r'[#@_$%\b\w\w+\b]{2,}', lowercase=False, max_features=1000)
...
# ------------------------------------- Decision Tree set model ------------------------------------
clf = tree.DecisionTreeClassifier(min_samples_leaf=int(len(train_tweet_text) * 0.01), criterion='entropy',
                                  random_state=0)
model = clf.fit(tweet_train_bag_of_words, train_sentiment)

# ------------------------------------- print results -------------------------------------
DT_predicted_sentiment = model.predict(tweet_test_bag_of_words)
for i in range(0, len(DT_predicted_sentiment)):
    print(test_instance_number[i], DT_predicted_sentiment[i])
